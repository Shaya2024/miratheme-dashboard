...
export async function GET(req) {
  try {
    ...
    return NextResponse.json({ payorDistribution: result.rows });
  } catch (error) {
    console.error("Error fetching payor distribution:", error);
    return NextResponse.json(
      { error: "Failed to fetch payor distribution data" },
      { status: 500 }
    );
  }
}
